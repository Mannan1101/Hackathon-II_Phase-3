...
export function ChatWidget({ onTodoChange }: ChatWidgetProps) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // Refresh todos if chatbot performed a todo operation
      if (response.metadata?.intent && onTodoChange) {
        const todoActions = ['add_task', 'delete_task', 'complete_task', 'update_task'];
        if (todoActions.includes(response.metadata.intent)) {
          onTodoChange();
        }
      }
    } catch (err) {
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50">
            {messages.length === 0 && (
              <div className="text-center text-gray-500 mt-20">
                ...
                <p className="text-sm">Ask me to manage your todos!</p>
